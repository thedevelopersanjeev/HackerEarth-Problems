bool isSafe(vector<vector<int>> &arr, int i, int j, int m, int n, vector<vector<bool>> &visited){
        return (i >= 0 && j >= 0 && i < m && j < n && arr[i][j] && !visited[i][j]);
    }
    
    void dfs(vector<vector<int>> &arr, vector<vector<bool>> &visited, int i, int j, int m, int n){
        visited[i][j] = true;
